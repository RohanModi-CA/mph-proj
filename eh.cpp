...
int main(void)
{
    ...
    Vector3 observer_abs_velocity = {12, 0.1, 0.0}; // now get the relative velocity
    ...
    auto rawloadedData = loadVector("events_data.bin");     std::vector<std::vector<float>> loadedData;     for (const auto& raw_corner_list : rawloadedData) {         loadedData.push_back( transformation( raw_corner_list.data(), observer_rel_velocity, 32 )  );     }
    ...
    {{
        ...
        BeginDrawing();
            ...
            BeginMode3D(camera);
                ...
                std::vector<float> corner_points = cube_vertices(3,2,5, 4);
                ...
                //DrawModel(model, block_pos, 1, GREEN);
                delete[] vect_points; //stop memory leaks
        /*
        if(frame_time > 1)
        {
            for (int i = 0; i < 24; i++) {
                std::cout << lorentzed_corners[i] << std::endl;
            }
        std::cout << "\n\n\n Now the non lorentz \n\n\n" << std::endl;
        for (int i = 0; i < 24; i++) {
            std::cout << corner_points[i] << std::endl;
        }
        return 1;
        }
        */
                ...
                int num_of_groups = 8;
                float *events_array = insert_t(corner_points.data(), num_of_groups, GetTime());
